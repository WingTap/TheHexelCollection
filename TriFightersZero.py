...
class AI:
    ...
    def discretize_state(self, opponent):
        dx = opponent.body.position.x - self.ship.body.position.x
        dy = opponent.body.position.y - self.ship.body.position.y
        dist = math.hypot(dx, dy)
        angle_to_enemy = math.atan2(dy, dx)
        ang_diff = angle_diff_signed(self.ship.angle, angle_to_enemy)
...
# --- Create ships and agents ---
player = Ship(100, HEIGHT//2, BLUE)
ai_ship = Ship(WIDTH-100, HEIGHT//2, RED)
ai_agent = AI(ai_ship, q_file="q_table.pkl")
ai_player = AI(player, q_file="q_table_blue.pkl")
...
while running:
    ...
    if action == "forward":
        if min_edge < 70:
            angc = angle_diff_signed(ai_ship.angle, angle_to_player)
            if abs(angc) > 0.2:
                if angc > 0:
                ...
                vx = math.cos(angle_to_player) * 220
                vy = math.sin(angle_to_player) * 220
    ...
    # --- Blue AI decision ---
    state_p = ai_player.discretize_state(ai_ship)
    action_p = ai_player.choose_action(state_p)

    dxp = ai_ship.body.position.x - player.body.position.x
    dyp = ai_ship.body.position.y - player.body.position.y
    angle_to_enemy_p = math.atan2(dyp, dxp)
    ang_diff_p = angle_diff_signed(player.angle, angle_to_enemy_p)
    min_edge_p = min(player.body.position.x, WIDTH - player.body.position.x,
                     player.body.position.y, HEIGHT - player.body.position.y)

    if action_p == "forward":
        if min_edge_p < 70:
            angc = angle_diff_signed(player.angle, angle_to_enemy_p)
            if abs(angc) > 0.2:
                if angc > 0:
                ...
                vx = math.cos(angle_to_enemy_p) * 220
                vy = math.sin(angle_to_enemy_p) * 220
                player.body.velocity = (vx, vy)
        else:
            player.move_forward(dt)
    elif action_p == "rotate_left":
        player.rotate(-1, dt=dt)
    elif action_p == "rotate_right":
        player.rotate(1, dt=dt)
    elif action_p == "shoot":
        if abs(ang_diff_p) < 0.35:
            player.shoot()
        else:
            if ang_diff_p > 0:
                player.rotate(1, dt=dt)
    ...
    next_state_p = ai_player.discretize_state(ai_ship)

    if ai_hit_player:
        reward += 100.0
        ai_agent.update_q(state, action, reward, next_state)
        ai_agent.save_q()
        ai_player.update_q(state_p, action_p, -100.0, next_state_p)
        ai_player.save_q()
        episode_count += 1
        ai_agent.epsilon = max(0.02, ai_agent.epsilon * 0.995)
        ai_player.epsilon = max(0.02, ai_player.epsilon * 0.995)
        reset_round()
        continue
    ...
    ai_agent.update_q(state, action, reward, next_state)
    ai_player.update_q(state_p, action_p, reward, next_state_p)
    ...
    if last_print > 120:
        last_print = 0
        print(f"Episode: {episode_count}, Eps Red: {ai_agent.epsilon:.3f}, "
              f"Eps Blue: {ai_player.epsilon:.3f}, "
              f"Q states Red: {len(ai_agent.q_table)}, Q states Blue: {len(ai_player.q_table)}")
